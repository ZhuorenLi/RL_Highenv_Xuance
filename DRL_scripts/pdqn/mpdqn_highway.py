...
from xuance.torch.agents import MPDQN_Agent
from high_d.src.highD_test import *


def parse_args():
    parser = argparse.ArgumentParser("Use mpdqn of XuanCe for highway.")
...
if __name__ == "__main__":
    parser = parse_args()
    configs_dict = get_configs(file_dir="mpdqn_highway.yaml")
    configs_dict = recursive_dict_update(configs_dict, parser.__dict__)
    configs = argparse.Namespace(**configs_dict)

    set_seed(configs.seed)
    envs = make_envs(configs)
    Agent = MPDQN_Agent(config=configs, envs=envs)
    ...
    if not configs.test:
        ...
        Agent.load_model(path=Agent.model_dir_load, model='mpdqn')
