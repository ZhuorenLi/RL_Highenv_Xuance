...
from xuance.torch.agents import MultiCritic_Attention_MPDQN_Agent
from xuance.extend_display.attention_display import Graphics
from high_d.src.highD_test import *


def parse_args():
    parser = argparse.ArgumentParser("Use multi-critic_att_mpdqn of XuanCe for highway.")
    parser.add_argument("--env-id", type=str, default="highway-v0")
    parser.add_argument("--test", type=int, default=0)
    parser.add_argument("--method", type=str, default="multi-critic_att_mpdqn")
...
if __name__ == "__main__":
    parser = parse_args()
    configs_dict = get_configs(file_dir="multi-critic_att_mpdqn_highway.yaml")
    ...
    Agent = MultiCritic_Attention_MPDQN_Agent(config=configs, envs=envs)
    ...
    if not configs.test:
        ...
        Agent.load_model(path=Agent.model_dir_load, model='multi-critic_att_mpdqn')
